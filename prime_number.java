...
A naive solution is for each number k <= n check whether k is divisible by any number less than k. If not, 
mark it as a prime number. Obviously, this algorithm takes O(n^2) time.

Let's take another view point of this problem.

Finding all prime numbers is equivalent to find the remaining numbers after removing non-prime numbers. That is, 
if we remove all even numbers, then all multiples of 3, all multiples of 5, ..., eventually, the remaining numbers 
...
Since n = sqrt(n) * sqrt(n), for any number greater than sqrt(n), it is either a prime number or it is a multiple of 
some number that is no greater than sqrt(n).
Proof: Suppose there is a number, say k<=n, such that it is not a prime number and it is > sqrt(n) and it is not a 
multiple of any number <= sqrt(n). Then it must has at least two divisors such that both of them are > sqrt(n). It 
implies that k > n, which is a contradiction to our assumption.
Similarly, to mark all multiples of i (except itself since it itself is a prime number :), we can start from i*i 
ince for any i*j, where 1<j<i, must have been taken care of in previous rounds.
...
Update (thanks to Keith): With these optimizations, each number will be marked k_i times, where k_i is the number 
f prime factors of the number i. E.g. 12 -- two times, one for 2 and the other for 3; 105 - 3 times, one for 3, 
one for 5 and one for 7.
In total, there are n numbers, for each prime x, we will mark n/x numbers, i.e. the total running time T(n) = 
n*(1/2 + 1/3 + 1/5 + ...) = n * sum_{i = all primes less than n} (1 / i). This series is called, Prime Harmonic Series, 
which has been proved to be asymptotic to loglogn. Therefore, the total running time T(n) = O(n*loglogn).
...
As proved in previous problem, instead of searching divisor among all numbers in (2..n), we only need to check numbers 
in [2..sqrt(n)].
*/
